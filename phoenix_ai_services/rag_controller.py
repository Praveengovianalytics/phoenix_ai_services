...
def get_inferencer(api_key: str, embedding_model: str, chat_model: str):
    embed_client = GenAIEmbeddingClient("openai", model=embedding_model, api_key=api_key)
...
def summarize_with_config(config, question, mode, top_k):
    try:
        rag = get_inferencer(config["api_key"], config["embedding_model"], config["chat_model"])
        df = rag.infer(
